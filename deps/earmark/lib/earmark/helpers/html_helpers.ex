defmodule Earmark.Helpers.HtmlHelpers do
  ...
  def add_attrs!(context, text, attrs_as_string_or_map, default_attrs, lnb) do
    with {context, {text, _errors}} <-
           add_attrs(context, text, attrs_as_string_or_map, default_attrs, lnb) do
      {context, text}
    end
  end
  ...
  defp add_attrs(context, text, nil, default, lnb),
    do: add_attrs(context, text, %{}, default, lnb)
