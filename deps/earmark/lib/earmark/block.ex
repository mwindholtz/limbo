defmodule Earmark.Block do
  ...
  import Earmark.Helpers.LookaheadHelpers,
    only: [opens_inline_code: 1, still_inline_code: 2, read_list_lines: 3]
  ...
  defmodule(ListItem,
    do: defstruct(lnb: 0, attrs: nil, type: :ul, spaced: true, blocks: [], bullet: "")
  )

  # prefix is appended to $$
  defmodule(Plugin, do: defstruct(lnb: 0, attrs: nil, lines: [], handler: nil, prefix: ""))
  ...
  defp _parse(
         lines = [
           %Line.TableLine{columns: cols1, lnb: lnb1},
  ...
  defp _parse(lines = [%Line.Text{lnb: lnb} | _], result, options) do
  ...
  defp _parse(
    ...
    {spaced, list_lines, rest, _offset, indent_level} =
      read_list_lines(rest, opens_inline_code(first), initial_indent)
    ...
    _parse(
      ...
      [%ListItem{type: type, blocks: blocks, spaced: spaced, bullet: bullet, lnb: lnb} | result],
      options1
    )
  end
  ...
  defp _parse([defn = %Line.IdDef{title: title, lnb: lnb}, maybe_title | rest], result, options)
    ...
    if title do
      ...
      _parse(
        [maybe_title | rest],
        [%IdDef{id: defn.id, url: defn.url, lnb: lnb} | result],
        options
      )
    end
  end
  ...
  defp _parse([defn = %Line.IdDef{lnb: lnb} | rest], result, options) do
    _parse(
      rest,
      [%IdDef{id: defn.id, url: defn.url, title: defn.title, lnb: lnb} | result],
      options
    )
  end
  ...
  defp _parse([defn = %Line.FnDef{id: _id, lnb: lnb} | rest], result, options) do
    ...
    {blocks, _, options2} =
      remove_trailing_blank_lines(indent_lines)
  ...
  defp _parse([%Line.Ial{attrs: attrs, lnb: lnb, verbatim: verbatim} | rest], result, options) do
    ...
    _parse(
      rest,
      [%Ial{attrs: attributes, content: attrs, lnb: lnb, verbatim: verbatim} | result],
      options1
    )
  end
  ...
  defp _parse(lines = [%Line.Plugin{prefix: prefix, lnb: lnb} | _], result, options) do
    ...
    if handler do
      _parse(
        rest1,
        [%Plugin{handler: handler, prefix: prefix, lines: plugin_lines, lnb: lnb} | result],
        options
      )
    else
      _parse(
        ...
        add_message(
          options,
          {:warning, lnb,
           "lines for undefined plugin prefix #{inspect(prefix)} ignored (#{lnb}..#{
             lnb + Enum.count(plugin_lines) - 1
           })"}
        )
      )
    end
  ...
  defp _consolidate_para([line | rest] = lines, result, pending) do
    case inline_or_text?(line, pending) do
      %{pending: still_pending, continue: true} ->
        _consolidate_para(rest, [line | result], still_pending)

      _ ->
        {result, lines, @not_pending}
    end
  end
  ...
  defp consolidate_list_items(
         [list = %List{type: type, blocks: items}, item = %ListItem{type: type} | rest],
         result
       ) do
  ...
  defp read_table(rest, col_count, %Table{rows: rows}) do
    ...
    table =
      case look_for_alignments(rows) do
        ...
        aligns -> %Table{table | alignments: aligns, header: hd(rows), rows: tl(tl(rows))}
      end
  ...
  defp find_closing_tags(needed = [needed_hd | needed_tl], [rest_hd | rest_tl], html_lines) do
    cond do
      closes_tag?(rest_hd, needed_hd) ->
        find_closing_tags(needed_tl, rest_tl, [rest_hd | html_lines])

      opens_tag?(rest_hd) ->
        find_closing_tags([rest_hd | needed], rest_tl, [rest_hd | html_lines])

      true ->
        find_closing_tags(needed, rest_tl, [rest_hd | html_lines])
    end
  end
  ...
  defp collect_plugin_lines(
         [%Line.Plugin{prefix: prefix, content: content, lnb: lnb} | rest],
         prefix,
         result
       ),
