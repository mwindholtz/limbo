defmodule Earmark.CLI do
  ...
  defp numberize_options(keywords, option_names),
    do: Enum.map(keywords, &numberize_option(&1, option_names))
    ...
    if Enum.member?(option_names, k) do
      case Integer.parse(v) do
        ...
        {int_val, rest} ->
          IO.puts(
            :stderr,
            "Warning, non numerical suffix in option #{k} ignored (#{inspect(rest)})"
          )
        ...
        :error ->
          IO.puts(
            :stderr,
            "ERROR, non numerical value #{v} for option #{k} ignored, value is set to nil"
          )
