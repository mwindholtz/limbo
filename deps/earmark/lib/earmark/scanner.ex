  ...
  defp scan_next_token(line, true) do
    cond do
      ...
      matches = Regex.run(@list_item_rgx, line) ->
        ...
        prefixed_with_ws(line, ws) || {make_list_item(bullet), behead(line, content), false}

      matches = Regex.run(@id_open_rgx, line) ->
        [_content, ws, id, rest] = matches
        prefixed_with_ws(line, ws) || {%IdOpen{id: id}, rest, false}
      ...
      matches = Regex.run(@code_fence_rgx, line) ->
        [_line, ws] = matches
        prefixed_with_ws(line, ws) || {%CodeFence{}, behead(line, 3), false}
